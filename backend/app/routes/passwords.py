...
from validators import (
    validate_password_data as xss_validate_password,
    SecurityValidator,
)
...
def log_audit_event(
    action, success=True, error_message=None, resource_id=None, user_id=None
):
...
def get_passwords(current_user):
    ...
    try:
        ...
        log_audit_event(
            "LIST_PASSWORDS", success=False, error_message=str(e), user_id=user_id
        )
        current_app.logger.error(
            f"Erreur lors de la récupération des mots de passe: {e}"
        )
    ...
    try:
        ...
        try:
            ...
            log_audit_event(
                "VIEW_PASSWORD",
                success=False,
                error_message=f"Decryption error: {str(decrypt_error)}",
                resource_id=password_id,
                user_id=user_id,
            )
    ...
    try:
        ...
        if errors:
            log_audit_event(
                "CREATE_PASSWORD",
                success=False,
                error_message=f"Validation: {'; '.join(errors)}",
                user_id=user_id,
            )
        ...
        try:
            ...
            log_audit_event(
                "CREATE_PASSWORD",
                success=False,
                error_message=f"Encryption error: {str(encrypt_error)}",
                user_id=user_id,
            )
        ...
        log_audit_event(
            "CREATE_PASSWORD", resource_id=password_entry.id, user_id=user_id
        )
        ...
        db.session.rollback()
        log_audit_event(
            "CREATE_PASSWORD", success=False, error_message=str(e), user_id=user_id
        )
...
def generate_password(current_user):
    ...
    try:
        ...
        log_audit_event(
            "GENERATE_PASSWORD", success=False, error_message=str(e), user_id=user_id
        )
...
def get_categories(current_user):
    ...
    try:
        ...
        categories_query = (
            db.session.query(
                Password.category, db.func.count(Password.id).label("count")
            )
            .filter(
            ...
            .group_by(Password.category)
            .order_by(Password.category)
            .all()
        )

        categories = [
            {"category": cat.category, "count": cat.count} for cat in categories_query
        ]
        ...
        log_audit_event(
            "LIST_CATEGORIES", success=False, error_message=str(e), user_id=user_id
        )
...
def get_presets(current_user):
    ...
    try:
        ...
        log_audit_event(
            "GET_PRESETS", success=False, error_message=str(e), user_id=user_id
        )
...
def evaluate_password_strength(current_user):
    ...
    try:
        ...
        log_audit_event(
            "EVALUATE_STRENGTH", success=False, error_message=str(e), user_id=user_id
        )
...
def update_password(current_user, password_id):
    ...
    try:
        ...
        password_obj = Password.query.filter_by(id=password_id, user_id=user_id).first()
        ...
        password_dict = {
            ...
            "password_changed_at": password_obj.password_changed_at.isoformat()
            if password_obj.password_changed_at
            else None,
            "last_used": password_obj.last_used.isoformat()
            if password_obj.last_used
            else None,
        }
